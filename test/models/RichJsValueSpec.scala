...
class RichJsValueSpec
  ...
  "set" - {
    ...
    "must return an error when trying to set an index out of bounds" in {
      ...
      value.set(path, JsString("fork")) mustEqual JsError(
        "array index out of bounds: 3, [\"bar\",\"baz\"]"
      )
    }
      ...
      forAll(gen) { case (key: String, values: List[String], indexToRemove: Int) =>
        ...
        val expectedOutcome =
          buildJsObj(
            Seq(key),
            Seq(
              valuesInArrays.slice(0, indexToRemove) ++ valuesInArrays.slice(
                indexToRemove + 1,
                values.length
              )
            )
